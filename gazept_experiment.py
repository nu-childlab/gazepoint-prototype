...
def main():
    ...
    for stim in stimuli_list:
        ...
        while True:
            ...
            break
        rt = timer.getTime()
        responsewriter.writerow({'subject':subject, 'response':response, 'rt':rt, 'image':stim.image, 'trial':trial})
        for row in tracking_results:
            trackingwriter.writerow({'subject':subject, 'tracking data':row, 'image':stim.image, 'trial':trial})
        trial += 1
...
def subjectCheck(subject):
	if re.search("^s\d*$", subject):
		...
		return subjectCheck(str(raw_input("Please enter a valid subject id: ")))
